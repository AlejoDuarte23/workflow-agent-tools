from app.types import Vec3, NodesDict, CombinationResult
import math
...
def get_nodes_by_z(nodes: NodesDict, z: float, tolerance: float = 1e-6) -> list[int]:
    ...
    return [
        ...
        if abs(node_data["z"] - z) < tolerance
    ]
...
def get_nodes_by_x(nodes: NodesDict, x: float, tolerance: float = 1e-6) -> list[int]:
    ...
    return [
        node_data["id"]
        for node_data in nodes.values()
        if abs(node_data["x"] - x) < tolerance
    ]


def get_nodes_by_x_and_z(
    nodes: NodesDict, x: float, z: float, tolerance: float = 1e-6
) -> list[int]:
