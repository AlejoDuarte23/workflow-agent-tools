...
Define the bridge geometry and cross-section parameters below.""")
    step_1.bridge_length = vkt.NumberField(
        "Bridge Length", min=100, default=20000, suffix="mm"
    )
    step_1.bridge_width = vkt.NumberField(
        "Bridge Width", min=100, default=4500, suffix="mm"
    )
    step_1.bridge_height = vkt.NumberField(
        "Bridge Height", min=100, default=3000, suffix="mm"
    )
...
Define the wind pressure to apply to the truss beam. The load will be visualized as point loads at nodes on the windward face (ZX plane at y=width, where z is between 0 and the truss height). Wind direction is in the positive Y direction.""")
    step_2.wind_pressure = vkt.NumberField(
        "Wind Pressure", min=0, default=1.5, suffix="kPa"
    )
...
Where **SLS** = Self-weight, **Q** = Gravitational load, **WL** = Wind load.""")
    step_3.deform_scale = vkt.NumberField(
        "Deformation Scale", min=1, max=500, default=25, step=1
    )
...
This button downloads the results in a JSON file. It gets the results for each combination along the model parameters""")
    step_3.download_btn = vkt.DownloadButton(
        "Download Results", method="download_results"
    )

    # Step 4: Sensitivity Analysis
    step_4 = vkt.Step(
        "Step 4 - Sensitivity Analysis", views=["show_sensitivity_analysis"]
    )
...
The analysis will vary the truss height from the minimum to maximum values and plot the results.""")
    step_4.min_height = vkt.NumberField(
        "Minimum Height", min=100, default=1000, suffix="mm"
    )
    step_4.max_height = vkt.NumberField(
        "Maximum Height", min=100, default=7000, suffix="mm"
    )
...
This button downloads the sensitivity analysis results in a JSON file. It includes the truss height values and corresponding maximum Z deformations.""")
    step_4.download_btn = vkt.DownloadButton(
        "Download Sensitivity Results", method="download_sensitivity_results"
    )
...
class Controller(vkt.Controller):
    ...
    def show_model(self, params, **kwargs):
        ...
        selected_cs = next(
            (cs for cs in cs_library if cs["name"] == selected_cs_name), None
        )
        ...
        cross_sections: CrossSectionsDict = {selected_cs["id"]: selected_cs}
    ...
    def download_results(self, params, **kwargs):
        ...
        if wind_pressure > 0:
            ...
            wind_nodes = [
                ...
                for node_id, node_data in nodes_dict.items()
                if (
                    abs(node_data["y"]) < 1e-6
                    and node_data["z"] >= -1e-6
                    and node_data["z"] <= height + 1e-6
                )
            ]
    ...
    def download_sensitivity_results(self, params, **kwargs):
        ...
        for height in height_values:
            ...
            if load_q > 0:
                ...
                load_cases.append(
                    {"name": "Dead Load", "factor": 1.0, "loads": dead_loads}
                )
            ...
            if wind_pressure > 0:
                ...
                load_cases.append(
                    {"name": "Wind Load", "factor": 1.0, "loads": wind_loads}
                )
