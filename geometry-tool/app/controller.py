...
from viktor.geometry import Point, Line, Material, RectangularExtrusion, SquareBeam
...
Please select a cross section size for the bridge members:""")
    ...
    download_btn = vkt.DownloadButton(
        "Download Bridge Geometry (JSON)", method="download_geometry_json"
    )
...
class Controller(vkt.Controller):
    ...
    def download_geometry_json(self, params, **kwargs):
        ...
        json_data = {
            ...
            "metadata": {
                ...
                "units": {"length": "millimeters", "cross_section": "millimeters"},
            },
        }
    ...
    def create_render(self, params, **kwargs):
        ...
        node1 = vkt.Point(
            params.bridge_length / 1000 + 0.4,
            -params.bridge_width / 1000,
            -height / 2 + cs_size,
        )
        node2 = vkt.Point(
            params.bridge_length / 1000 + 0.400,
            2 * params.bridge_width / 1000,
            -height / 2 + cs_size,
        )
        center_line = vkt.Line(node1, node2)
        ...
        deck = SquareBeam(
            deck_length, deck_width, deck_thickness, material=deck_material
        )
        deck.translate(
            (deck_length / 2, deck_width / 2, deck_thickness / 2)
        )  # Position at z=0
        ...
        asphalt = SquareBeam(
            deck_length, deck_width, asphalt_thickness, material=asphalt_material
        )
        asphalt.translate(
            (
                deck_length / 2,
                deck_width / 2,
                deck_thickness / 2 + asphalt_thickness / 2,
            )
        )
        ...
        data = vkt.DataGroup(
            geometry_params=vkt.DataItem(
                ...
                subgroup=vkt.DataGroup(
                    ...
                    divisions=vkt.DataItem(
                        "Number of Divisions", int(params.n_divisions)
                    ),
                ),
            ),
            model_stats=vkt.DataItem(
                "Model Statistics",
                "",
                subgroup=vkt.DataGroup(
                    nodes=vkt.DataItem("Total Nodes", len(nodes)),
                    members=vkt.DataItem("Total Members", len(lines)),
                    total_length=vkt.DataItem(
                        "Total Member Length",
